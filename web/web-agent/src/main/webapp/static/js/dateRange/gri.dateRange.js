...
*/
	...
	function createQueryRange(dateObj) {
		this._date = new pickerDateRange(dateObj.inputId, {
            aToday : dateObj.aToday || 'aToday', //今天
            aYesterday : dateObj.aYesterday || 'aYesterday', //昨天
            aRecent7Days : dateObj.aRecent7Days || 'aRecent7Days', //最近7天
            aRecent14Days : dateObj.aRecent14Days || 'aRecent14Days',//最近14天
            aRecent30Days : dateObj.aRecent30Days || 'aRecent30Days', //最近30天
            aRecent90Days : dateObj.aRecent90Days || 'aRecent90Days', //最近90天
			theme : 'ta', // 日期选择器TA主题
			autoCommit : true, //自动提交，完成日期初始化，以及图表的展示拉取
			isTodayValid : dateObj.isTodayValid,
			startDate : dateObj.startDate,
			endDate : dateObj.endDate,
			success : dateObj.success,
			...
			'<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf">',
				...
				'<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
					...
					'<div class="frm_btn">',
						...
						'<input class="ta_btn" type="button" id="' + this.closeBtn + '" value="Cancel"/>',
					'</div>',
				'</div>',
			'</div>'
		]
	...
	gri:[
		'<label class="gri_contrast" for ="' + this.compareCheckboxId + '">',
            '<input type="checkbox" class="gri_pc" name="' + this.compareCheckboxId + '" id="' + this.compareCheckboxId + '" value="1"/>',
        '</label>',
	...
	ta:[
		'<label class="contrast" for ="' + this.compareCheckboxId + '">',
            '<input type="checkbox" class="pc" name="' + this.compareCheckboxId + '" id="' + this.compareCheckboxId + '" value="1"/>',
        '</label>',
